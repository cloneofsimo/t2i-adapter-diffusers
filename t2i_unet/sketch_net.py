...
def sketch_extracter():
    model = pidinet()
    ckp = torch.hub.load_state_dict_from_url(
        "https://raw.githubusercontent.com/TencentARC/T2I-Adapter/main/models/table5_pidinet.pth"
    )["state_dict"]
...
if __name__ == "__main__":
    model = sketch_extracter()
    im = Image.open("./contents/test.png").convert("RGB").resize((512, 512))
    im = np.array(im).astype(np.float32) / 255.0
    im = im.transpose(2, 0, 1).reshape(1, 3, 512, 512)
